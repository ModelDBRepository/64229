: $Id: intf.mod,v 1.1.1.1 2005/12/15 15:16:39 hines Exp $
...
static unsigned int jtpt,jitmax,invlmax;
...
NEURON {
  ...
  RANGE taum,myinvl,oinvl,WINV,invlt         :::: interval bursting params
...
CONSTRUCTOR {
  ...
  { int lid,lty;
    ...
    IDP = (id0*)hoc_Ecalloc(1, sizeof(id0)); hoc_malchk();
    ip = IDP;
    ip->id=lid; ip->type=lty; 
    ip->invl = ip->record = ip->jitter = ip->input = 0; // all flags off
    ip->rve=-1;
  }
  ENDVERBATIM
}
...
NET_RECEIVE (wAM,wNM,wGA,wGB,wflg) { LOCAL tmp,rflg,wrec,id,jflg,iflg,invlflg,ty
  ...
  _linvlflg=(double)ip->invl; _lwrec=(double)ip->wrec; _lid=(double)ip->id; 
  _lty=(double)ip->type; 
  ENDVERBATIM
  if (flag==4) { : mid-burst
...
PROCEDURE initinvl () {
  ...
  {int i, err=0;
    if (! ifarg(1)) {printf("Return initinvl(vec)\n"); return(0.);}
    invlmax=vector_arg_px(1, &invlp);
    if (invlmax==0) {err=1; printf("initinvl ERR: vec not initialized\n");}
    if (err) { invlmax=-1; invlp=nil; return(0.); }// hoc_execerror("",0);
  }
  ENDVERBATIM
}
...
FUNCTION invlflag () {
  VERBATIM
  ip=IDP;
  if (ip->invl==1 && invlp==nil) { // err
    printf("INTF invlflag ERR: pointer not initialized\n"); hoc_execerror("",0); 
  }
  _linvlflag= (double)ip->invl;
  ENDVERBATIM
}
...
//if (invlp==nil) {printf("INTF invlflag ERRa: pointer not initialized\n"); hoc_execerror("",0);}
  if ((t<(invlt-myinvl)+myinvl/2) && invlt != -1) { // don't shift if less than halfway through
...
PROCEDURE chk () {
  ...
  {int i,lfg;
  ip=IDP;
  printf("ID:%d; typ: %d; rec:%d wrec:%d inp:%d jit:%d invl:%d\n",ip->id,ip->type,ip->record,ip->wrec,ip->input,ip->jitter,ip->invl);
  ...
  if (lfg==4) { 
    if (invlp!=nil) { printf("INVLP: (%d)\n",invlmax);
      for (i=0;i<invlmax;i++) printf("%d:%g  ",i,invlp[i]);
      printf("\n");
    } else printf("no INVLP\n");
  }
  if (lfg==5) { 
...
FUNCTION invlset () {
  VERBATIM
  ip=IDP;
  if (ifarg(1)) ip->invl = (short) *getarg(1);
  _linvlset=(double)ip->invl;
  ENDVERBATIM
}
